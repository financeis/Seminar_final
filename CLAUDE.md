...
코드 수정 전 docs/standards.md와 해당 모듈의 AGENTS.md를 읽는다. 수식·가정 변경은 docs/idea_paper.pdf 원문과 docs/business-rules.md의 명시적 가정을 확인한다. 데이터·달력 변경은 결정·체결·목표 끝을 펼쳐 비교한다. 이전 감사의 ‘진단만’ 범위는 종료됐으며 새 패키지 수정은 허용됐다.

실제 시장 자료와 합성 반례, 코드 구현과 실제 실행을 구별한다. 오래된 결과를 최신 소스의 실행으로 인증하지 않는다. PCA 설명력을 예측 정확도라고 부르지 않는다. 현재 원자료와 재구현 실험의 출처를 보존한다. 구 구현·감사 파일은 사용자 요청으로 현재 체크아웃에서 제거했으며 Git 이력에만 남아 있다.
