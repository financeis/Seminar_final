...
def validate_config(c: ResearchConfig) -> ResearchConfig:
    ...
    check(0 <= r.missing_rate < 1 and 0 <= r.ffill_limit <= 2, 'invalid missing-data policy: forward fill is limited to two months')
    check(0 < r.pca_variance <= 1 and r.k_normal == 5, 'research requires five normal regimes and a valid PCA threshold')
